...
module.exports = function() {
  ...
  router.get('/fashion', async (req, res) => {
    try {
      const filePath = path.join(__dirname, 'database', 'fashion.json');
      const data = await fs.readFile(filePath, 'utf8');
      const fashion = JSON.parse(data || '[]');
      res.json(fashion);
    } catch (err) {
  ...
  router.post('/save-fashion-data', upload.single('photo'), async (req, res) => {
    const newData = {
    ...
    try {
      ...
      res.redirect('/fashion.html'); // Redirect to the grocery.html page
    } catch (err) {
